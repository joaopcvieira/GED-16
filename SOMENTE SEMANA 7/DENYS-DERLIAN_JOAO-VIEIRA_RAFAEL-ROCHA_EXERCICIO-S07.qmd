...
```{r}
model1_data <- model1_data |>
    mutate(log_milk_consumption_pc = milk_consumption_pc^0.25)
```
...
```{r}
cor(model1_data$birth_rate, model1_data$log_milk_consumption_pc)
```
...
```{r}
cor(model1_data$expected_schooling, model1_data$log_milk_consumption_pc)
```

É possível perceber que existe correlação entre as variáveis explicativas. A correlação mais forte é entre as variáveis "birth_rate" e "expected_schooling". Além disso, a variável `log_milk_consumption_pc` possui suas correlações reduzida com a variável `expected_schooling` e praticamente inalterada com a variável `birth_rate` se comparada com a variável não transformada `milk_consumption_pc`.
...
Para a variável transformada `log_milk_consumption_pc`

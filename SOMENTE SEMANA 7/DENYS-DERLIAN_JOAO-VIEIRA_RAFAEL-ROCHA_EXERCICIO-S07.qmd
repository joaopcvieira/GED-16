...
É possível observar que as variáveis `milk_consumption_pc` e `life_expectancy` não aparentam possuir uma relação linear. Com isso o modelo construído será com uma transformação da variável `milk_conumption_pc`.

```{r}
model1_data <- model1_data |>
  mutate(new_milk_consumption_pc  = milk_consumption_pc^0.25) 
```
...
Percebe-se que a variável `new_milk_consumption_pc`possui uma relação mais próxima de linear que `milk_consumption_pc` com a variável resposta `life_expectancy`.
...
É possível perceber que existe correlação entre as variáveis explicativas. A correlação mais forte é entre as variáveis "birth_rate" e "expected_schooling". Além disso, a variável `new_milk_consumption_pc` possui suas correlações reduzida com a variável `expected_schooling` e praticamente inalterada com a variável `birth_rate` se comparada com a variável não transformada `milk_consumption_pc`.
...
Conclui-se que para todas as variáveis quantitativas do modelo a correlação entre seus valores lineares e seus valores quadráticos é muito elevada. Para diminuir esse efeito no modelo, centraliza-se as variáveis explicativas antes da construção do modelo.
...
model1_quant <- lm(
    life_expectancy ~ I(birth_rate_c^2) + I(expected_schooling_c^2) + I(milk_consumption_pc_c^2) + birth_rate_c * expected_schooling_c + birth_rate_c * milk_consumption_pc_c + expected_schooling_c * milk_consumption_pc_c,data = model1_data)
...
<<<<< ALGUMA CONTINUAÇÃO AQUI >>>>>>



```{r}
# modelo que inclui as variáveis explicativas qualitativas
m_quant <- lm(
    life_expectancy ~ birth_rate_c * expected_schooling_c, data = model1_data
)
```

## Inclusão da variável categórica `continent`

Vamos avaliar se o modelo de regressão melhora com a inclusão da variável categórica `continent`.


```{r}
# modelo que inclui as variáveis explicativas qualitativas
m_quali <- lm(
    life_expectancy ~ birth_rate_c * expected_schooling_c + continent, data = model1_data
)
...
A partir do resumo do modelo, é possível observar que a variável `continent` agrega valor ao modelo, tendo em vista que o valor p do teste F é menor que 0.05. 

A fim de verificar isso, vamos realizar o teste ANOVA entre o modelo quando este não incluia a variável categórica e agora, incluindo a variável `continent`.

```{r}
# Teste ANOVA
anova(m_quant, m_quali, test="F")

```

De fato, o valor expresso para o teste F mostra que a variável `continent` agrega valor ao modelo.

## Inclusão de termos de interação entre variáveis quantitativas e qualitativas
Verificando gráficamente se há interação entre as variáveis `birth_rate_c` e `expected_schooling_c` com a variável categórica `continent`. 
...
Os gráficos apresentados mostram que aparentemente a interação entre as variáveis `birth_rate_c` e `expected_schooling_c` com a variável `continent` não são relevantes. Para definir se isso quantitativamente é adotado o procedimento a seguir:

```{r}
# Criando modelo de regressão com todas as interações possíveis
m_all <- lm(
    life_expectancy ~ birth_rate_c*expected_schooling_c + continent:birth_rate_c + continent : expected_schooling_c + continent, data = model1_data 
)
```

Verificando se a inclusão das interações melhora o não o modelo:

```{r}
# Verificando a inclusao das novas alterações
anova(m_quali,m_all, test="F")
```


Ao observar a estatística do teste F é possível concluir que a inclusão das interações entre as variáveis quantitativas e a variável categórica `continent` não traz benefício para o modelo criado. Assim, o modelo a ser utilizado será o modelo `m_quali` sem a inclusão das interações.

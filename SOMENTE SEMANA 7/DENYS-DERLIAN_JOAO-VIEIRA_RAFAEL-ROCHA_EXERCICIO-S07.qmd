...
$E[life\_expectancy] = \beta_0 + \beta_1 new\_milk\_consumption\_pc + \beta_2 birth\_rate  + \beta_3 expected\_schooling + \beta_{33} expected\_schooling^2 + \beta_{12}new\_milk\_consumption\_pc\cdot birth\_rate$

```{r}
# Modelo reduzido
model1_quant_red <- lm(
    life_expectancy ~ birth_rate_c + expected_schooling_c +I(expected_schooling_c^2) + new_milk_consumption_pc_c +  birth_rate_c * new_milk_consumption_pc_c, data = model1_data)


summary(model1_quant_red)
```

O modelo reduzido é significativo (passa no teste-F global com valor-p da ordem de e-16). Todos os coeficientes são significativos e o coeficiente de determinação indica que aproximadamente 85.2% da variabilidade na resposta é explicada pelo modelo (não há grande diferença, se comparado ao coeficiente de determinação ajustado).
...
Agora, analisa-se o impacto do modelo de regressão com a inclusão da variável categórica `continent`:
